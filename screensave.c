...
int WINAPI WinMain (HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmdLine, int iCmdShow)
{
	...
	{
		char buf[200];
		wsprintf(buf, "%d %d %s %d %d %d %d %d %d\n",
	...
	switch (chOption) {
	case 's':
		return ScreenSaver(NULL);			// screen save
	case 'p':
		return ScreenSaver(hwndParent);		// preview in small window
	case 'a':
		return ChangePassword(hwndParent);	// ask for password
	case 'c':
		return Configure(hwndParent);		// configuration dialog
	default:
		return Configure(hwndParent);		// configuration dialog
	}
