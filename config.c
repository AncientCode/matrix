...
WNDPROC OldProc;
HRESULT CALLBACK PreviewProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	PAINTSTRUCT ps;
	RECT rect;
	switch(uMsg)
	...
	case WM_PAINT:
		...
		FillRect(ps.hdc, &rect, GetStockObject(BLACK_BRUSH));
		EndPaint(hwnd, &ps);
		return 0;
	}
...
DWORD WINAPI PreviewThread(PVOID hwnd)
{
	ScreenSaver(hwnd);
	return 0;
}
...
BOOL CALLBACK ConfigDlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		// colour sliders
		SendDlgItemMessage(hwnd, IDC_SLIDER_H, TBM_SETRANGE, 0, MAKELONG(0, 255));
		SendDlgItemMessage(hwnd, IDC_SLIDER_H, TBM_SETPOS, TRUE, g_nHue);


		CheckDlgButton(hwnd, IDC_CYCLE, g_fColorCycle);
	...
	case WM_COMMAND:
		...
		case IDC_CYCLE:
			g_fColorCycle = IsDlgButtonChecked(hwnd, IDC_CYCLE);
			break;
